...
const dirSetup = [{
			"Folder": "admin", "CmdName": "admin",
			"CmdDescription": "Grant specific admin about something!"
	},{
			"Folder": "fun", "CmdName": "fun",
			"CmdDescription": " the fun of the Bot!"
	},{
			"Folder": "economy", "CmdName": "economy",
			"CmdDescription": "economics"
	},{
			"Folder": "general", "CmdName": "general",
			"CmdDescription": "general Commands"
	},{
			"Folder": "rank", "CmdName": "rank",
			"CmdDescription": "leveling rank Commands"
	}/*{     
...
module.exports = (bot) => {
    try {
		let allCommands = [];
        readdirSync("./C-slash/").forEach((dir) => {
if(lstatSync(`./C-slash/${dir}`).isDirectory()) {
				const groupName = dir;
...
//add the subcommand to the array
				...
				if (pull.name && pull.description) {
					let Command = new SlashCommandBuilder().setName(String(pull.name).toLowerCase()).setDescription(pull.description);
						...
						bot.slash.set("normal" + pull.name, pull)
				} 
				else {
					console.log(file, `error -> missing a help.name, or help.name is not a string.`.brightRed);
				}
