  ...
  run: async (interaction, bot, data) => {
    ...
    if (status !== "on" && status !== "off") {
      return interaction.reply({
        content: `Please specify a valid value between **on** and **off**`,
      });
    }
    ...
    if (status === "on") {
      ...
      interaction.reply({
        content: `Autorole enabled! New members will automatically receive the **${role.name}** role.`,
      });
    ...
    if (status === "off") {
      if (!data.guild.plugins.autorole.enabled) {
        return interaction.reply({
          content: `**The autorole is already disabled.**`,
        });
      }
